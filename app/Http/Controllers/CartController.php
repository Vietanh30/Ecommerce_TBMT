...
class CartController extends Controller
{
    ...
    public static function cartUpdate(Request $request)
    {
        ...
        if ($request->quant) {
            ...
            foreach ($request->quant as $k => $quant) {
                $id = $request->qty_id[$k]; // Lấy ID giỏ hàng
                $cart = Cart::find($id); // Tìm giỏ hàng theo ID

                // Nếu giỏ hàng và số lượng hợp lệ
                if ($quant > 0 && $cart) {
                    ...
                    // Tính lại số tiền giỏ hàng sau khi cập nhật số lượng
